...
FMT_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
    -> To {
  ec = 0;
  using FType = std::numeric_limits<From>;
  using T = std::numeric_limits<To>;
  static_assert(FType::is_integer, "From must be integral");
  static_assert(T::is_integer, "To must be integral");

  // A and BType are both signed, or both unsigned.
  if (detail::const_check(FType::digits <= T::digits)) {
...
FMT_CONSTEXPR auto lossless_integral_conversion(const From from, int& ec)
  ...
  if (detail::const_check(FType::is_signed && !T::is_signed)) {
